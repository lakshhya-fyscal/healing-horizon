...
import {
  CREAM, CREAM2, CREAM3, FOREST, SAGE, SAGE_LT, SAGE_MD, TERRA, TERRA_LT, GOLD,
  MUTED, WHITE, BORDER,
  D, B,
  BotanicalRight, BotanicalLeft,
  GlobalStyle, useReveal, Navbar, Footer,
} from "./theme.jsx";
...
const SERVICES = [
  ...
  },
  {
    icon: "🧘",
    title: "Grounding Kit",
    desc: "Don't want to commit to therapy but would like to learn how to manage it? Download these 5 strategies that will give you immediate relief.",
    tags: ["Self-Paced", "Digital Download", "Anxiety Relief"],
    durationLabel: "Format",
    duration: "Digital Download",
    formatLabel: "Delivery",
    format: "Instant Access",
    color: "rgba(196,148,90,0.16)",
    accent: GOLD,
    badge: "Digital Product",
    badgeColor: GOLD,
    cta: { label: "Get the Grounding Kit", href: "https://superprofile.bio/vrindagoel/jugFvgNhhe" },
  },
];
            ...
            <div key={i} className={`card reveal`} style={{
              ...
              {s.badge && (
                <div style={{
                  ...
                  background: s.badgeColor || TERRA, color: WHITE,
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontFamily: B, fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: "0.1em" }}>{s.durationLabel || "Duration"}</div>
                  <div style={{ fontFamily: B, fontSize: 13, fontWeight: 600, color: FOREST, marginTop: 2 }}>{s.duration}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: B, fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: "0.1em" }}>{s.formatLabel || "Format"}</div>
                  <div style={{ fontFamily: B, fontSize: 13, fontWeight: 600, color: FOREST, marginTop: 2 }}>{s.format}</div>
                </div>
              </div>

              {s.cta && (
                <a href={s.cta.href} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{
                  display: "block", textAlign: "center", marginTop: 22, fontSize: 13.5, padding: "13px 24px",
                }}>{s.cta.label}</a>
              )}
            </div>
          ))}
